...
**注意**: AdCP の元リポジトリ ([adcontextprotocol/adcp](https://github.com/adcontextprotocol/adcp)) は別組織が管理しているため、最新版へのアップデートは手動で行う必要があります。Dockerfile のビルドプロセスで最新の `main` ブランチをクローンしますが、セキュリティ上の理由から、重要な更新時には手動でデプロイを実行することを推奨します。
...
GitHub Actions の「Actions」タブから手動でデプロイをトリガー：

1. https://github.com/miyaichi/adcp-gcloud-deploy/actions にアクセス
...
**重要**: AdCP は別組織が管理しているため、セキュリティ上の理由から手動デプロイを推奨します。ビルド時に最新の AdCP コードがクローンされるため、変更内容を確認してからデプロイしてください。

### 自動デプロイ（オプション）

`main` ブランチへのプッシュで自動的にデプロイすることも可能です：
...
**注意**: 自動デプロイでは、ビルド時に AdCP の最新版がクローンされるため、予期せぬ変更が含まれる可能性があります。本番環境では手動デプロイを推奨します。
